...
- **代码注释规范**：
  - 默认不要主动添加注释；只有当业务意图、边界条件、生命周期约束、跨进程边界或安全限制无法从代码本身直接看出时，才允许补充必要注释
  - 严禁编写“翻译代码”的低价值注释，例如“定义变量”“调用接口”“更新状态”“发送 IPC 请求”这类从代码字面即可判断的描述
  - 优先通过更准确的命名、合理的函数拆分、清晰的类型定义和更小的职责边界提升可读性；能通过重构消除注释需求时，优先重构，不要堆叠注释
  - 注释应主要解释“为什么这样实现”以及“不这样做会出现什么问题”，而不是重复“这段代码正在做什么”
  - 对 Electron 主进程 / 渲染进程职责分界、Pi 会话生命周期、流式事件映射、资源释放、持久化约束、第三方技能安全边界等易误用逻辑，应补充简短且准确的说明性注释
  - 修改代码时必须同步检查相关注释；凡是失效、误导、与当前实现不一致或带有历史遗留语气的注释，必须一并删除或改写，不能保留
  - 除示例文档、协议原文或外部接口字段外，项目内新增代码注释统一使用简体中文；单条注释尽量控制在 1-2 行，避免长段落解释
  - 若 AI 生成的代码包含模板化注释、空泛注释、显而易见的标题注释或与当前实现无关的占位注释，视为无效输出，必须在提交前清理
